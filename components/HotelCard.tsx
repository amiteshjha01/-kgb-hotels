...
export default function HotelCard({ title, description, image, link, tag, tagVariant = "gold", features = [], phone1, phone2, whatsappPhone, whatsappMsg, featured = false, guestTypes }: HotelCardProps) {
    const waLink = `https://wa.me/91${whatsappPhone || "9676247755"}?text=${encodeURIComponent(whatsappMsg || `Hi, I'd like to enquire about ${title}`)}`

    return (
        <div className={`luxury-card flex flex-col overflow-hidden group ${featured ? "ring-1 ring-gold-400/25 shadow-[0_0_40px_rgba(212,175,55,0.07)]" : ""}`}>
            {featured && (
            ...
            <div className="flex flex-col flex-grow p-6">
                <h3 className="font-serif text-xl font-semibold text-white mb-1 group-hover:text-gold-300 transition-colors duration-300">{title}</h3>
                ...
                <div className="space-y-2.5 mt-auto">
                    <a href={waLink} target="_blank" rel="noopener noreferrer" className="wa-btn w-full text-sm py-3">
                        <WA_ICON /> Enquire via WhatsApp
                    </a>
